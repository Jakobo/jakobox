const items = [
  "Welcome to the box! A silly little corner of the internet, with a bar of silly facts.",
  "Chat Rules: Keep it clean, stay respectful, don't be an asshole",
  "My current favorite weapon is an Arminius-D with Crowd Control & Focused Fire",
  "The cat's name is Churchill =^_^= He likes to chew on HDMI cables...",
  "When not streaming, I work for a Technology company in San Francisco",
  "Vault of Glass: [x]  // Crota's End [ ] // Prison of Elders: [ ]",
  "King's Fall: [x] // Challenge of Elders [x]",
  "Wrath of the Machine: [ ]",
  "Best Trials Card: [7] wins",
  "I play Dungeons & Dragons with my coworkers. We're currently playing Out of the Abyss.",
  "If I had to pick a favorite alcohol, it'd be bourbon.",
  "Have feedback on the stream? Whisper/message me. I'm always looking to improve!",
  "Without a regular schedule, the best way to catch me is to follow the stream.",
  "I love absolutely terrible movies. Like, really terrible movies...",
  "In \"Behind the Stream\" in Creative, we'll dig into the application behind this stream.",
  "When not sporting UR, I'll often take a scout rifle into the Crucible."
];

export default items;
